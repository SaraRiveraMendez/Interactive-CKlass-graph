...
# =============================
st.markdown(
    '<div class="section-title">Top 3 Productos en Ventas — Temporada</div>',
    unsafe_allow_html=True,
)
...
# =============================
# TOP 3 PRODUCTOS NEGADOS
# =============================
st.markdown(
    '<div class="section-title">Top 3 Productos Negados — Temporada</div>',
    unsafe_allow_html=True,
)
...
if top3.empty:
    st.info("No hay datos de negados para mostrar el ranking.")
else:
    MEDALLAS = ["🥇", "🥈", "🥉"]
    ...
    cols = st.columns(3)
    for i, row in top3.iterrows():
        pct = (row["Negados"] / total_sucursal * 100) if total_sucursal > 0 else 0
        barra_fill = int(pct)  # % para la barra visual
